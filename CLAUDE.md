...
`package.json` 中的版本号遵循 semver（`MAJOR.MINOR.PATCH`）。当前处于 0.x 阶段（pre-1.0），规则如下：

| 层级 | 触发条件 | 示例 |
|---|---|---|
| **PATCH** `0.x.Y` | Bug 修复、文案修正、类型补丁、小重构 | `0.2.0` → `0.2.1` 修复显示错位 |
| **MINOR** `0.Y.0` | 新功能、新表/迁移、新页面/路由、向后兼容的能力扩展 | `0.2.0` → `0.3.0` 新增报名草稿流程 |
| **MAJOR** `1.0.0` | 正式发布（v1 赛季稳定运行后） | `0.x` → `1.0.0` |

**每次 dev → main 合并必须递增版本号**，由合并者判断层级。合并后打 tag `vX.Y.Z` 指向 main 的 merge commit。

**禁止手动修改 `package.json` 中的 version 字段。** 必须使用：

```bash
npm version patch   # 0.3.0 → 0.3.1
npm version minor   # 0.3.0 → 0.4.0
npm version major   # 0.x   → 1.0.0
```

`npm version` 会同时更新 `package.json`、创建 git commit、打 git tag。之后推送：

```bash
git push --atomic origin dev vX.Y.Z
```

1.0.0 之前不保证 API 稳定，但每次 main 合并都需经过 `pnpm build` + `pnpm test` 全绿。
...
| 鉴权 | Supabase Auth（email+password + magic link）+ iron-session（双 Cookie：`rivalhub-session` 全用户 + `rivalhub-admin` root 紧急） |
