...
- **DB 冷启动自愈**：Vercel 函数冷启动时 `DATABASE_URL` 偶发为空/残值导致 pg Pool 绑定错误地址，现连接失败自动重建 Pool、重读环境变量并重试当次查询，并发重建合并去重
- **OCR 日志可诊断性**：`siliconflow.ts` 每个处理步骤设独立 try/catch + 日志，失败时精确定位
