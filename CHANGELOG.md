...
- 15316af: 修复已开放赛事保存报名运营时间时对客户端有损回放的误判，并明确区分计划开放、实际开放与提前强制开放语义。
- b9b02e6: 修复信息与反馈入口与页脚的桌面空间冲突，补齐学信网认证驳回后的重新提交引导，并校准社区奖、选手资料和邀请码页面的宽度语义。
...
新增全局「信息与反馈」入口，并支持全站及单届赛事公告。公告可区分普通通知、产品更新和重要提醒，并支持草稿、发布、撤回及需要用户主动关注的提醒。

赛事页面新增统一的公开信息入口，可集中展示赛事规则、交流群、群二维码和公开联系方式。

新增站内反馈功能，登录用户和未登录用户均可提交问题、建议与内容纠错，赛事管理员可统一处理反馈。

### Fixed

#### 报名换队与退赛流程

完善赛事名单调整期间的个人退赛与换队流程。审核通过但最终名单尚未锁定时，选手现在可以主动退出本届赛事；原审核名单继续保留为历史记录，当前名单重新进入确认与审核流程。

修复选手已经确认代表原队参赛、同时收到新队邀请时，报名页面错误优先显示新邀请并导致无法完成换队的问题。页面现在会优先展示当前已确认的参赛队伍，并明确引导选手先退出原赛事名单；完成退出后即可继续处理新队邀请。

#### 教育认证审核材料

修复超级管理员查看录取通知书材料时可能只打开空白标签页的问题。现在材料通过受保护的服务端链接打开，并重定向至短期私有访问地址。
...
修复绑定第二邮箱时对 Supabase 签发的邮箱确认 OTP 类型识别与验证，避免有效确认链接被错误拒绝。
...
赛事草稿现在可以继续调整 slug、赛事模板、公开规则、赛程和报名规则；创建赛事时 slug 会持续跟随名称生成，直到管理员主动编辑。纯中文名称无法自动生成 URL 标识时，页面会明确要求填写合法 slug；已存在的草稿也可以修改 slug，并在保存后跳转到新的管理地址。

赛事发布后，slug、赛事模板和公开赛事规则保持稳定；实际开放报名后，本届竞技上下文、实际开放时间和临时 5E fallback 规则冻结。比赛开始前，报名截止时间和名单调整截止时间仍可由运营人员调整。

#### 竞技平台历史赛季维护

管理员新增历史竞技赛季时使用更紧凑的 Dialog，以赛季名称为主要输入，并在保存前预览其时间顺序。稳定技术标识收纳到高级设置，减少日常维护时的干扰。

### Fixed

#### 管理员访问反馈

管理员访问现在能区分未登录与权限不足；权限校验或数据加载出现意外错误时，会保留正确的错误语义，不再误导至登录页。

#### 赛事规则展示

修复赛事规则页的 Markdown 展示，正确呈现标题、列表、引用、链接和 GFM 表格，并保持页面主标题唯一。
...
新增 `/teams/recruitment` 组队大厅。

队长可以为长期 Team 发布招募信息，选择需要的位置和可选目标赛事；选手可以发布找队信息并展示自己的常用位置。选手可向感兴趣的队伍表达加入意向，队长确认后仍通过既有 Team Invitation 流程完成正式入队。

组队意向只用于发现与撮合，不创建新的成员关系或赛事报名事实；长期 Team、TeamMembership、CompetitionEntry 的既有边界保持不变。

#### 社区奖与赛后解说

新增赛事社区奖完整流程，支持：

- 用户提交社区奖名称、条件、奖品与补充说明；
- 赛事管理员审核、要求补充或驳回；
- 公开奖项提交候选人、相关比赛、说明和视频证据；
- 管理员记录获奖、本届不颁或取消，并保留结果更正审计。

社区奖与冠军、亚军、MVP、EVP 等正式赛事荣誉保持独立，不写入 `tournament_honors`。

比赛现在可以登记最多两名实际解说，并记录赛后录像和赛后资料提交状态。解说可以在个人资料中维护直播间链接；待开赛和进行中的公开比赛页会展示当前解说的直播入口。

### Changed

#### 赛事发布与报名窗口

赛事“发布”和“开放报名”现在是两个独立事实。
...
管理员可以分别配置报名开放时间、报名截止时间和名单调整截止时间。

报名截止后不再接受新的 CompetitionEntry，但已经存在的参赛队伍可以在名单调整窗口内继续完成允许的本届名单变更；长期 Team 本身不会因为某届赛事报名截止而被冻结。

#### Major 竞技参考策略

Major 不再在赛事发布时冻结竞技平台赛季，而是在**实际开放报名的同一事务中**冻结本届竞技参考上下文。

默认实力参考调整为：

**历史最高段位 × 50% + 前一完整赛季最高段位 × 20% + 近期最高段位 × 30%**

其中近期最高段位取最近完整赛季与当前进行中赛季中的较高值。

这使平台刚切换新赛季时，玩家不会因为新赛季样本过少而被迫短期冲分，同时近期明显提升的竞技水平仍然能够反映在赛事资格与种子参考中。

#### Team 与个人资料体验

优化长期 Team 的入口与公开页面：

- 无队伍用户可以更明确地创建队伍或进入组队大厅；
- 队伍目录和公开主页统一展示语言；
- 恢复并优化队伍图标维护入口；
- 调整队伍页面导航与任务层级。

选手公开主页现在可以展示已经审核通过的高校身份认证。

### Fixed

- 优化竞技档案的段位与 S 段星数填写布局，使星数紧邻对应段位。
- 强化竞技位置和教育认证学校选择后的视觉反馈，并支持重新选择学校。
- 统一注册确认、当前邮箱验证和密码重置邮件的 RivalHub 品牌样式、安全提示与两步邮箱确认说明。
- 补充赛事窗口状态的公开提示和管理员发布确认，减少“发布赛事”“开放报名”“正式开赛”之间的语义混淆。
- 收紧名单调整边界：只有赛事实际开放报名后，已批准参赛队伍才能自助调整名单。

### Security

- 社区奖、证据、解说和赛后报告继续由 server-only 数据库 owner 读写；public Data API 的 anon/authenticated 访问被明确拒绝。

### Migration & Operations

本版本包含新的 forward migrations，用于社区奖与赛后解说、组队大厅、赛事参与窗口、名单调整 revision 来源及其 server-only 数据库访问保护。
...
- 排行榜和个人页面因 OCR+Demo 双录导致的 maps 数双倍计数，过滤 `source=manual_ocr`
- 队伍页面选手 maps 数双倍计数（同图 OCR+Demo 双录），补充 `source=manual_ocr` 过滤，与排行榜/个人页对齐
...
- **文档全面精简**：error-reference(540→31 行)、data-model(443→291)、code-map(74→29)、architecture(163→145)，合并 ui-design + ui-tokens → ui-system.md，归档 launch-readiness
...
- **Bracket 空槽位显示**：`BracketView` 在 brackets-viewer 渲染后注入对阵来源标签——UB R2+ 显示「队伍 A vs 队伍 B 胜者」，LB 各轮显示「胜者/败者」，未确定则显示 TBD；同时把对阵卡片加宽到 280px、原生 hint 翻译为中文
...
- `formatCSTDateTime()` 格式化函数（CST 月日+时间，如 "5 月 18 日 19:30"）

### Changed

- **比赛时间显示日期+具体时间**：MatchCard 和赛季首页 NEXT MATCHES 从仅日期（"5 月 18 日"）改为日期+时间（"5 月 18 日 19:30"）
...
- **MVP 次数统计修正**：选手个人页 MVP 计数从「总票数」改为「单场 MVP 获奖次数」；新增 `matches.mvp_winner_user_id` 持久化缓存，避免每次页面访问遍历全表投票记录；UI 标签改为「单场 MVP」
