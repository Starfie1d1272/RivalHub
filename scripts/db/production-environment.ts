...
export const PRODUCTION_POOLER_PORT = "5432";
...
/** Vercel production builds inherit the already-configured runtime DB URL. */
export function buildVercelProductionVerificationEnvironment(env: Environment = process.env): NodeJS.ProcessEnv {
  ...
  const databaseUrl = assertProductionDatabaseUrl(env.DATABASE_URL);
  return {
    ...env,
    NODE_ENV: "production",
