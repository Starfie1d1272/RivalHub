...
try {
  ...
  const result = spawnSync(executable, ["scripts/db/steam-profile-remediation.ts", ...process.argv.slice(2)], {
    cwd: projectRoot,
    stdio: "inherit",
    env: {
      ...environment,
  ...
  if (result.signal) throw new Error(`Steam profile remediation 被信号 ${result.signal} 终止。`);
  if (result.status !== 0) throw new Error(`Steam profile remediation 失败（exit ${result.status ?? "unknown"}）。`);
} catch (error) {
