...
import { expirePendingInvitationsInTx } from "../../src/lib/teams/invitations";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // accept 侧：pending 但已过期的邀请按 id 收敛为 expired 后，不再满足
    // accept 的 pending 前置条件，无法形成 membership。
    await pool.query("UPDATE team_invitations SET expires_at = now() - interval '1 minute' WHERE id = $1", [ids.fresh]);
    const acceptedExpired = await database.transaction((tx) =>
      expirePendingInvitationsInTx(tx, { invitationId: ids.fresh }),
    );
    check(acceptedExpired === 1, "accept 路径的按 id 过期收敛应命中一封。");
    ...
    check(memberships.rows[0]?.count === "0", "接受过期邀请不能形成 membership。");
    ...
    console.log("Team invitation lifecycle local integration passed: stale pending invite expiry, freed pending identity for re-invite, untouched fresh invites, and no membership from expired invitations.");
  } finally {
