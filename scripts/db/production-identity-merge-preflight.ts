...
try {
  ...
  const result = spawnSync(executable, ["scripts/db/identity-merge-preflight.ts", ...process.argv.slice(2)], {
    cwd: projectRoot,
    stdio: "inherit",
    env: {
      ...environment,
  ...
  if (result.signal) throw new Error(`Identity merge preflight 被信号 ${result.signal} 终止。`);
  if (result.status !== 0) throw new Error(`Identity merge preflight 失败（exit ${result.status ?? "unknown"}）。`);
} catch (error) {
