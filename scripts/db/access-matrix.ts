...
export const DATABASE_ACCESS_MATRIX: readonly DatabaseAccessEntry[] = [
  ...
  serverOnly(
    ...
    "身份 / credential",
    "高敏感邮箱绑定挑战",
    "src/lib/identity/linking.ts; src/actions/identity.ts",
    "绑定请求包含邮箱和一次性挑战状态，只能在服务端验证与消费。",
  ),
  serverOnly(
    "major_final_results",
    "Major post-event",
  ...
  serverOnly(
    ...
    "Major prestart",
    "高敏感冻结竞技事实与来源",
    "src/lib/major/prestart-entrants.ts; src/lib/major/prestart-seeds.ts; src/lib/admin/season-workspace/major-prestart.ts",
    "系统建议快照绑定冻结 entrant/EventRoster、竞技 provenance 与 ConversionPolicy，只由服务端 freeze/read model 访问，不能通过 Data API 暴露。",
  ),
  serverOnly(
  ...
  serverOnly(
    ...
    "src/lib/identity/; src/actions/identity.ts",
    "credential 绑定、撤销与归并只经服务端 identity owner，公开页面不读取原始标识。",
  ),
  serverOnly(
  ...
  serverOnly(
    ...
    "身份 / 数据归并",
    "高敏感双重 identity 控制证明",
    "src/lib/identity/linking.ts; src/lib/identity/merge.ts",
    "self-service merge 只消费短期、服务端验证的双重控制授权，不能由客户端伪造。",
  ),
  serverOnly(
    "user_merge_ledger",
    "身份 / 数据归并",
    "高敏感 canonical alias 与归并审计",
    "src/lib/identity/merge.ts; src/actions/user-merge.ts",
    "old user 到 canonical user 的 durable alias、证据类别和 domain summary 仅供服务端解析与审计。",
  ),
  serverOnly(
    "user_sessions",
    "鉴权 / 会话",
