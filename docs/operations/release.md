...
workflow dispatch 只能从当前 `main` HEAD 发起；preflight 会冻结该 operator SHA、验证其 canonical `main` push CI，并拒绝从其它 branch/ref 或过期 main SHA 发起的 retry。满足该 operator integrity gate 后，可以 retry 同一个已存在 tag，但分两种情况：
