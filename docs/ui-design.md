...
Hero + CURRENT SEASON 两栏（lg 以下堆叠）。Hero 含 Eyebrow mono 小标 + Display 大标题 + 操作按钮组；CURRENT SEASON 面板含 MiniStat 三格（TEAMS / PLAYERS / STAGE）。下方 nav tiles `grid-cols-2 lg:grid-cols-4` 8 个固定入口，hover 时 border 变亮。其他赛季 `grid-cols-1 sm:grid-cols-2 lg:grid-cols-3`；无活跃赛季时显示 `EmptyState`。

---

## 赛季首页 `/[seasonSlug]`

SeasonNav（ScrollHint 横滚渐变遮罩）→ 标题区（StatusPill + 赛季大标题）→ Phase Tracker（capability 驱动 + ScrollHint，当前阶段 accent 色，已完成 ✓ 绿色）→ NEXT MATCHES 面板（`status === "playing"` 时展示，最近 4 场 scheduled/in_progress，`formatCSTShortDate` 显示日期，LIVE 状态绿色 mono 标注）→ Quick Links（1/2/4 列响应）→ Stat 四格（`<Stat>` 组件，TEAMS / PLAYERS / MATCHES / STAGE，MATCHES 格式 `finished/total`，无比赛显示 `—`）。
...
**详情**：首发阵容 + 替补分区，队长 badge 标识；战绩/数据 grid 响应式折行，阵容行 truncate 防溢出。
...
- `/admin/[seasonSlug]/matches`：比赛列表 + 录入比分表单（两个 number input + 提交按钮）。
