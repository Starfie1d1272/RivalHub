...
`competition_entry_restriction_overrides` 是 Entry 审核中的显式政策决定 ledger：每条记录绑定一个 Entry、当前 `rosterRevisionId`、typed qualification finding 的 `restrictionCode` 与完整 finding snapshot，另存具体理由、授予人/时间和可审计的撤销人/时间。它只允许解除 `waivable=true` 的当前政策限制；身份、资料缺失和 roster/state 完整性 finding 不可被该表替代。新的 roster revision 不继承旧 revision 的有效解除。
