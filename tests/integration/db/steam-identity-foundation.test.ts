...
describe("Steam identity foundation", () => {
  it("keeps a changed primary as a revocable historical gameplay identity", async () => {
    ...
    try {
      ...
      const [currentUser] = await database.select({ steam64: schema.users.steam64 }).from(schema.users).where(eq(schema.users.id, userId));
      expect(currentUser?.steam64).toBe(nextSteam64);
  ...
  it("uses excluded values on cache conflict to update official profile", async () => {
