    ...
    await withScratchDatabase("rivalhub_conversion_policy_admin", async (client: Client) => {
      ...
      const frozenConfig = JSON.stringify({
        ...
        competitiveProfile: {
          ...
          fallbackConversion: { sourcePlatform: "fivee", version: base.version, seasonKeyMap: {} },
        },
      });
      for (const [suffix, openedAt] of [["published", null], ["open", "2026-09-07T00:00:00Z"]] as const) {
        await client.query(
          ...
          [randomUUID(), `conversion-policy-${suffix}-${randomUUID()}`, `Conversion Policy ${suffix}`, frozenConfig, openedAt],
        );
      }
