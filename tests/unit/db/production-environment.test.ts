...
describe("production database target guard", () => {
  ...
  it("fails closed for missing confirmation, staging targets and unapproved writes", () => {
    ...
    expect(() => buildProductionEnvironment({ ...confirmation, RIVALHUB_PRODUCTION_DB_HOST_CONFIRM: "aws-0-ap-northeast-1.pooler.supabase.com:6543" }, { requiresWriteAuthorization: false })).toThrow(/DB_HOST_CONFIRM/);
  ...
  it("rejects inherited or non-production URLs and only lets Vercel production builds inherit the runtime URL", () => {
    ...
    expect(() => assertProductionDatabaseUrl("postgresql://postgres.cueazphyskstwdhnzsxx:pw@aws-0-ap-northeast-1.pooler.supabase.com:6543/postgres")).toThrow(/固定的 production/);
    ...
    expect(buildVercelProductionVerificationEnvironment({ VERCEL_ENV: "production", DATABASE_URL: runtimeUrl })).toMatchObject({
      RIVALHUB_DB_TARGET: "production",
      RIVALHUB_PRODUCTION_PROJECT_CONFIRM: PRODUCTION_PROJECT_REF,
    });
  });
