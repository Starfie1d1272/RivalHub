...
interface Props { seasonId: string; seasonName: string; currentUserId: string; minTeamSize: number; maxTeamSize: number; application: TeamApplicationView | null; members: TeamApplicationMemberView[]; qualification?: { njuPrimaryCount: number; externalStrength: { state: "pending" | "pass" | "fail"; blockers: string[] } }; }
...
export function TeamApplicationFlow({ seasonId, seasonName, currentUserId, minTeamSize, maxTeamSize, application, members, qualification }: Props) {
  ...
  const [name, setName] = useState(application?.name ?? ""); const [perfectTeamId, setPerfectTeamId] = useState(application?.perfectTeamId ?? ""); const [primaryStarterUserIds, setPrimaryStarterUserIds] = useState<string[]>(application?.primaryStarterUserIds ?? []); const [inviteEmail, setInviteEmail] = useState("");
  ...
  return <div className="space-y-5"><StatusBanner tone={application.status === "approved" ? "success" : application.status === "rejected" ? "warn" : "info"} title={`${application.name} · ${STATUS_LABEL[application.status]}`} sub={application.reviewReason ?? (editable ? "按顺序完善下列报名检查后提交审核。" : "报名资料已锁定，等待管理员审核。")} />
    <Panel label="1 · 队伍资料" pad={24}><div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_auto]"><div className="space-y-2"><Label htmlFor="application-name">队伍名称</Label><Input id="application-name" value={name} disabled={!editable} onChange={(event) => setName(event.target.value)} /><div className="space-y-2"><Label htmlFor="perfect-team-id">完美战队 ID</Label><Input id="perfect-team-id" value={perfectTeamId} disabled={!editable} onChange={(event) => setPerfectTeamId(event.target.value)} placeholder="报名审核与完美平台赛事房间核对使用" /></div></div><div className="space-y-2"><Label>队标</Label><TeamLogoUpload applicationId={application.id} currentLogoUrl={application.logoUrl} teamName={application.name} canEdit={editable} /></div></div>{editable && <Button className="mt-4" variant="outline" disabled={isPending || !name.trim()} onClick={() => run(() => updateTeamApplication({ applicationId: application.id, name, perfectTeamId, primaryStarterUserIds }), "队伍资料已保存")}>保存队伍资料</Button>}</Panel>
