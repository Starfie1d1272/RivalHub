...
interface Props {
  ...
  maxRoster: number;
  starterCount: number;
  requiresCompetitiveProfile: boolean;
  showsPerfectTeamId: boolean;
...
export function CompetitionEntryFlow(props: Props) {
  ...
  const recruitmentHint = props.minRoster === 5 && props.maxRoster === 9 && props.starterCount === 5
    ? <p className="text-sm leading-6 text-[var(--color-fg-mid)]">{props.approvedTeamCount >= 32 ? `当前已有 ${props.approvedTeamCount} 支队伍通过报名审核，仍可继续报名；若最终超过 32 支，将按赛事规则进行资格赛。也可以加入已有队伍或担任替补。` : "还没有完整阵容？可以加入已有队伍或担任替补。"}每队可报名 5–9 人，合理准备替补有助于应对比赛时间冲突。<Link className="ml-2 underline underline-offset-4 focus-visible:ring-2 focus-visible:ring-[var(--color-accent)]" href={`/teams/recruitment?event=${props.competitionId}`}>查看招募中的队伍</Link></p>
    : null;

  if (!props.entry) {
    if (!props.capabilities.canStartRegistration) return <StatusBanner tone="info" title={props.capabilities.readOnlyReason ?? "报名尚未开放"} />;
    if (props.captainedTeams.length === 0) return <Panel contentClassName="p-6">{recruitmentHint}<StatusBanner tone="info" title="先创建队伍" sub="报名需要由一支你担任队长的队伍发起。队伍人数可以之后再补齐。" /><Button asChild className="mt-4"><Link href="/my/teams">前往我的队伍</Link></Button></Panel>;
  ...
  const blockers = [
    ...
    ...(props.requiresTeamLogo ? [{ label: entry.logoUrl ? "队伍图标已上传" : "队伍图标未上传：请到我的队伍上传，再保存本届名单", state: entry.logoUrl ? "complete" as const : "blocked" as const }] : []),
