...
export function CompetitionEntryFlow(props: Props) {
  ...
  return <div className="space-y-5">
    ...
    {representative && <>
      ...
      <Panel label="2 · 成员确认" pad={24}><div className="space-y-2">{entry.roster.map((member) => <div key={member.participantId} className="flex items-center justify-between border border-[var(--color-border)] px-3 py-2 text-sm"><span>{member.label}{member.primary ? " · 预定主力" : ""}</span><span className={member.confirmation === "confirmed" ? "text-[var(--color-ok)]" : "text-[var(--color-warn)]"}>{member.confirmation === "confirmed" ? "已确认" : "待本人确认"}</span></div>)}</div>{own?.confirmation === "invited" && <Button className="mt-4" disabled={pending} onClick={() => run(() => confirmCompetitionEntryParticipation({ entryId: entry.id }), "你已确认本届参赛")}>确认我本人参赛</Button>}</Panel>
