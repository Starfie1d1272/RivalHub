...
export function CompetitionEntryFlow(props: Props) {
  ...
  const missingTeamLogoLabel = props.canManageEntryTeamProfile
    ? "队伍图标未上传：请到我的队伍上传，再保存本届名单"
    : "队伍图标未上传：请联系当前队长在“我的队伍”中上传，再保存本届名单";
  const blockers = [
    ...(unsaved ? [{ label: "名单或队伍 ID 有未保存的修改，请先保存本届名单", state: "blocked" as const }] : []),
    ...(props.requiresTeamLogo ? [{ label: entry.logoUrl ? "队伍图标已上传" : missingTeamLogoLabel, state: entry.logoUrl ? "complete" as const : "blocked" as const }] : []),
