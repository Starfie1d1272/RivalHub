  ...
  return <div className="space-y-5">
    ...
    return <Panel key={entry.id} label={`报名审核 · ${entry.name}`} contentClassName="p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div><div className="flex flex-wrap items-center gap-2"><h3 className="text-lg font-semibold">{entry.name}</h3><Badge variant="outline">{presentCompetitionEntryRegistration(entry.status).label}</Badge><Badge variant="outline">{entry.source === "linked_team" ? "长期队伍报名" : "赛事组队"}</Badge></div><p className="mt-2 text-sm text-[var(--color-fg-mid)]">负责人：{entry.representativeName} · 完美战队 ID（可选）：{entry.perfectTeamId ?? "未填写"}</p></div>
