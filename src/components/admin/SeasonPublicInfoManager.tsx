...
export function SeasonPublicInfoManager({ data }: { data: SeasonPublicInfoAdmin }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-4 border border-[var(--color-border)] bg-[var(--color-panel-low)] p-5">
        <div><h2 className="text-lg font-semibold text-[var(--color-fg)]">交流群</h2><p className="mt-1 text-sm text-[var(--color-fg-mid)]">填写群号或加入链接创建交流群；创建后可上传或更换二维码。</p></div>
...
function GroupEditor({ item, index, total, pending, run }: { item: GroupRow; index: number; total: number; pending: boolean; run: (work: () => AsyncResult, message: string) => void }) {
  ...
  const canEnable = item.status === "active" || Boolean(form.groupNumber.trim() || form.joinUrl.trim());
  const save = () => run(() => updateCommunityGroup({ id: item.id, ...form, status: item.status }), "交流群已更新");
  return <div className="border border-[var(--color-border)] bg-[var(--color-panel)] p-4"><div className="grid gap-3 md:grid-cols-2"><Field label="名称" value={form.label} onChange={(value) => setForm((current) => ({ ...current, label: value }))} /><Field label="面向人群" value={form.audience} onChange={(value) => setForm((current) => ({ ...current, audience: value }))} /><Field label="群号" value={form.groupNumber} onChange={(value) => setForm((current) => ({ ...current, groupNumber: value }))} /><Field label="加入链接" value={form.joinUrl} onChange={(value) => setForm((current) => ({ ...current, joinUrl: value }))} /><div className="space-y-2 md:col-span-2"><Label>说明</Label><Textarea rows={2} value={form.note} onChange={(event) => setForm((current) => ({ ...current, note: event.target.value }))} /></div><div className="flex items-end gap-2 text-sm"><span className={item.status === "active" ? "text-[var(--color-ok)]" : "text-[var(--color-fg-dim)]"}>{item.status === "active" ? "启用中" : "已关闭"}</span>{item.qrImagePath && <span className="text-[var(--color-fg-dim)]">已有二维码</span>}</div>{item.status === "closed" && !canEnable && <p className="text-xs text-[var(--color-fg-mid)] md:col-span-2">重新启用前，请先填写群号或加入链接。关闭群后，旧二维码不会自动恢复。</p>}</div><div className="mt-3 flex flex-wrap items-center gap-2"><Button size="sm" disabled={pending || !form.label.trim() || (item.status === "active" && !form.groupNumber.trim() && !form.joinUrl.trim() && !item.qrImagePath)} onClick={save}>保存</Button><Button size="sm" variant="outline" disabled={pending || index === 0} onClick={() => run(() => moveCommunityGroup(item.id, "up"), "已上移")}>上移</Button><Button size="sm" variant="outline" disabled={pending || index === total - 1} onClick={() => run(() => moveCommunityGroup(item.id, "down"), "已下移")}>下移</Button><Button size="sm" variant="outline" disabled={pending || !canEnable} title={item.status === "closed" && !canEnable ? "请先填写群号或加入链接" : undefined} onClick={() => run(() => updateCommunityGroup({ id: item.id, ...form, status: item.status === "active" ? "closed" : "active" }), item.status === "active" ? "交流群已关闭" : "交流群已启用")}>{item.status === "active" ? "关闭交流群" : "重新启用"}</Button>{item.qrImagePath && <Button size="sm" variant="outline" disabled={pending} onClick={() => run(() => removeCommunityGroupQr(item.id), "二维码已删除")}>删除二维码</Button>}<Button size="sm" variant="outline" disabled={pending} onClick={() => run(() => deleteCommunityGroup(item.id), "交流群已删除")}>删除</Button><label className={`inline-flex h-9 items-center border border-[var(--color-border)] px-3 text-sm ${item.status === "closed" ? "cursor-not-allowed opacity-50" : "cursor-pointer hover:bg-[var(--color-panel-hi)]"}`}>上传/更换二维码<input className="sr-only" type="file" accept="image/jpeg,image/png,image/webp" disabled={pending || item.status === "closed"} onChange={(event) => { const file = event.target.files?.[0]; event.currentTarget.value = ""; if (!file) return; if (file.size > LOGO_MAX_BYTES) { toast.error("二维码图片不能超过 1 MB。 "); return; } if (!(LOGO_ALLOWED_TYPES as readonly string[]).includes(file.type)) { toast.error("请上传 JPG、PNG 或 WebP 格式的图片。 "); return; } const formData = new FormData(); formData.set("file", file); run(() => uploadCommunityGroupQr(item.id, formData), "二维码已更新"); }} /></label></div></div>;
}
