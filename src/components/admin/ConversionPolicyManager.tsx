    ...
    <Panel label="策略版本目录" contentClassName="p-0">
            ...
            {newDraft && (
              <>
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2"><Label>基准策略</Label><Select value={newDraft.basePolicyId} onValueChange={(value) => setNewDraft({ ...newDraft, basePolicyId: value })}><SelectTrigger><SelectValue placeholder="选择已有策略" /></SelectTrigger><SelectContent>{approvedBases.map((policy) => <SelectItem key={policy.id} value={policy.id}>{policy.version} · {policyStatusLabel(policy.status)}{policy.isCurrent ? " · 当前" : ""}</SelectItem>)}</SelectContent></Select></div>
      ...
      <Dialog open={Boolean(selected && editor)} onOpenChange={(open) => { if (!open) closePolicy(); }}>
        <DialogContent size="xl">
          ...
          {confirmation && selected && <div className="px-4 pb-4 sm:px-6"><InlineConfirm danger={confirmation === "retire"} title={confirmation === "approve" ? "批准这份 policy 版本？" : confirmation === "set-current" ? "将这份 policy 设为当前版本？" : "退役这份历史 policy？"} sub={confirmation === "set-current" ? "已发布或已开放赛事的锁定版本不会被修改。" : confirmation === "retire" ? "退役后不能恢复或编辑，但历史赛事引用仍可读取。" : "批准后 mapping 与版本身份将不可修改。"} confirmLabel="确认操作" onCancel={() => setConfirmation(null)} onConfirm={confirmLifecycleAction} /></div>}
        </DialogContent>
      </Dialog>
    </Panel>
  );
...
function StarSegmentEditor({ segment, index, editable, onChange, onRemove, canRemove }: { segment: StarSegment; index: number; editable: boolean; onChange: (index: number, key: keyof StarSegment, value: string) => void; onRemove: (index: number) => void; canRemove: boolean }) {
  return <div className="rounded-sm border border-[var(--color-border)] p-3"><div className="mb-3 flex items-center justify-between gap-2"><span className="text-xs font-semibold text-[var(--color-fg-mid)]">分段 {index + 1}</span>{editable && <Button type="button" size="sm" variant="ghost" disabled={!canRemove} onClick={() => onRemove(index)}>删除</Button>}</div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><NumberField label="minStar" value={segment.minStar} disabled={!editable} onChange={(value) => onChange(index, "minStar", value)} /><NumberField label="maxStar（最后一段可空）" value={segment.maxStar ?? ""} disabled={!editable} onChange={(value) => onChange(index, "maxStar", value)} /><div className="space-y-2"><Label>目标段位</Label><Select disabled={!editable} value={segment.targetRank} onValueChange={(value) => onChange(index, "targetRank", value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{perfectWorldRanks.map((rank) => <SelectItem key={rank.rankKey} value={rank.rankKey}>{rankLabel(rank)}</SelectItem>)}</SelectContent></Select></div><NumberField label="targetStarFloor（无星目标可空）" value={segment.targetStarFloor ?? ""} disabled={!editable} onChange={(value) => onChange(index, "targetStarFloor", value)} /><NumberField label="slopeNum" value={segment.slopeNum} disabled={!editable} onChange={(value) => onChange(index, "slopeNum", value)} /><NumberField label="slopeDen" value={segment.slopeDen} disabled={!editable} onChange={(value) => onChange(index, "slopeDen", value)} /></div></div>;
}
...
function NumberField({ label, value, disabled, onChange }: { label: string; value: number | string; disabled: boolean; onChange: (value: string) => void }) {
  return <div className="space-y-2"><Label>{label}</Label><Input type="number" value={value} disabled={disabled} onChange={(event) => onChange(event.target.value)} /></div>;
}
