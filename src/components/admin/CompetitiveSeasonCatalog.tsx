...
export function CompetitiveSeasonCatalog({ rows }: { rows: CompetitiveCatalogRow[] }) {
  ...
  const save = () => startTransition(async () => {
    ...
    if (result.success) { toast.success("平台赛季目录已保存"); setDraft(blank()); router.refresh(); } else toast.error(result.error.message);
  });
  const remove = (id: string) => startTransition(async () => { const result = await deleteCompetitivePlatformSeason(id); if (result.success) { toast.success("平台赛季目录已删除"); router.refresh(); } else toast.error(result.error.message); });
