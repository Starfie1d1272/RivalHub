...
export function PositionEditor({ value, disabled, onChange }: { value: string[]; disabled?: boolean; onChange: (value: string[]) => void }) {
  ...
  return <div className="space-y-3">
    ...
    <div className="flex flex-wrap gap-2">
      {CS2_POSITION_VALUES.map((position) => <label key={position} className="flex items-center gap-2 rounded border border-[var(--color-border)] px-3 py-2 text-sm">
    ...
    {value.filter((position) => !(CS2_POSITION_VALUES as readonly string[]).includes(position)).length > 0 && <div className="flex flex-wrap gap-2">
      {value.filter((position) => !(CS2_POSITION_VALUES as readonly string[]).includes(position)).map((position) => <span key={position} className="inline-flex items-center gap-2 rounded border border-[var(--color-border)] px-3 py-1.5 text-sm">{position}<Button type="button" variant="ghost" size="sm" disabled={disabled} onClick={() => onChange(value.filter((item) => item !== position))}>删除</Button></span>)}
    </div>}
