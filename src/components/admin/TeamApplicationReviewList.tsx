...
export interface TeamApplicationReviewRow {
  ...
  members: Array<{ userId: string; email: string; displayName: string | null; perfectId: string | null; emailVerified: boolean; educationStatus: "unsubmitted" | "pending" | "approved" | "rejected"; institutionName: string | null; status: "invited" | "confirmed" }>;
...
export function TeamApplicationReviewList({ applications }: { applications: TeamApplicationReviewRow[] }) {
  ...
  return <div className="space-y-3">{applications.map((application) => {
    ...
    return <Card key={application.id} className="p-4"><div className="flex items-start justify-between gap-4"><div className="min-w-0 space-y-2"><div className="flex items-center gap-2"><h3 className="font-semibold">{application.name}</h3><Badge variant="outline">{LABEL[application.status]}</Badge></div><p className="text-sm text-[var(--color-fg-mid)]">队长：{application.captainEmail} · 已确认 {confirmed}/{application.members.length} · 完美战队 ID：{application.perfectTeamId ?? "未填写"}</p><p className="text-xs text-[var(--color-fg-mid)]">预定主力：{application.primaryStarterUserIds.length}/5</p><div className="space-y-1 text-xs text-[var(--color-fg-mid)]">{application.members.map((member) => <p key={member.userId}>{member.displayName ?? member.email} · {member.status === "confirmed" ? "已确认" : "待确认"} · {member.emailVerified ? "邮箱已验证" : "邮箱未验证"} · {member.institutionName ?? "未声明学校"} · {member.educationStatus} · Perfect ID {member.perfectId ?? "未填写"}{application.primaryStarterUserIds.includes(member.userId) ? " · 预定主力" : ""}</p>)}</div>{application.reviewReason && <p className="text-sm text-[var(--color-warn)]">审核说明：{application.reviewReason}</p>}</div>{(application.status === "submitted" || application.status === "waitlisted") && <div className="flex shrink-0 flex-col gap-1.5"><Button size="sm" disabled={isPending} onClick={() => review(application.id, "approved")}>通过</Button><Button size="sm" variant="outline" disabled={isPending} onClick={() => review(application.id, "waitlisted")}>候补</Button><Button size="sm" variant="ghost" disabled={isPending} onClick={() => review(application.id, "rejected")}>拒绝</Button></div>}</div></Card>;
  })}</div>;
}
