...
const LABEL = { draft: "待完善", submitted: "待审核", approved: "已通过", waitlisted: "候补", rejected: "需修改后重提" } as const;
...
export function TeamApplicationReviewList({ applications }: { applications: TeamApplicationReviewRow[] }) {
  ...
  const review = (applicationId: string, status: "approved" | "waitlisted" | "rejected") => {
    const reason = status === "rejected" ? window.prompt("请填写需要队长修改后重提的原因：")?.trim() : undefined;
    if (status === "rejected" && !reason) return;
    startTransition(async () => { const result = await reviewTeamApplication({ applicationId, status, reason }); if (!result.success) toast.error(result.error.message); else toast.success(status === "approved" ? "已通过并生成正式队伍" : status === "rejected" ? "已退回修改" : `报名队伍已${LABEL[status]}`); });
  };
  ...
  return <div className="space-y-5">{applications.map((application) => {
    ...
    return <Panel key={application.id} label={`报名审核 · ${application.name}`} pad={0}><div className="space-y-5 p-4 sm:p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="text-lg font-semibold">{application.name}</h3><Badge variant="outline">{LABEL[application.status]}</Badge></div><p className="mt-2 break-all font-mono text-[11px] text-[var(--color-fg-mid)]">队长：{application.captainEmail} · 完美战队 ID：{application.perfectTeamId ?? "未填写"}</p></div>{(application.status === "submitted" || application.status === "waitlisted") && <div className="flex flex-wrap gap-2"><Button size="sm" disabled={isPending} onClick={() => review(application.id, "approved")}>通过报名</Button><Button size="sm" variant="outline" disabled={isPending} onClick={() => review(application.id, "waitlisted")}>候补</Button><Button size="sm" variant="destructive" disabled={isPending} onClick={() => review(application.id, "rejected")}>退回修改</Button></div>}</div>
