...
export function PreMatchOperatorChecklist({ teamA, teamB, mapState, requiresPreflight = false }: { teamA: PreMatchTeamState; teamB: PreMatchTeamState; mapState: "not_recorded" | "recorded" | "not_required"; requiresPreflight?: boolean }) {
  ...
  const teamItems = (team: PreMatchTeamState): ChecklistItem[] => [
    ...
    { label: `${team.name} · 赛事资格复核`, state: !requiresPreflight || team.preflight?.valid ? "complete" : "blocked", detail: !requiresPreflight ? "本场不是 Major 托管比赛，不需要额外的 Major 资格预检。" : team.preflight?.valid ? "冻结名单、NJU 首发、外校实力、纪律与身份均通过当前服务端预检。" : team.preflight?.blockers.join(" ") ?? "先提交并确认合法首发后才能预检。" },
  ];
  ...
  const rosterBlockers = [teamA, teamB].flatMap((team) => !team.submitted ? [`${team.name} 尚未提交首发`] : team.starters !== 5 ? [`${team.name} 当前不是 5 名首发`] : !team.confirmed ? [`${team.name} 首发尚未确认`] : requiresPreflight && !team.preflight?.valid ? team.preflight?.blockers.map((blocker) => `${team.name}：${blocker}`) ?? [`${team.name} 尚未得到服务端预检结果`] : []);
