...
export interface PreMatchTeamState { name: string; submitted: boolean; confirmed: boolean; starters: number; }
...
export function PreMatchOperatorChecklist({ teamA, teamB, mapState }: { teamA: PreMatchTeamState; teamB: PreMatchTeamState; mapState: "not_recorded" | "recorded" | "not_required" }) {
  ...
  const teamItems = (team: PreMatchTeamState): ChecklistItem[] => [
    ...
    { label: `${team.name} · 赛事资格复核`, state: team.confirmed && team.starters === 5 ? "pending" : "blocked", detail: team.confirmed && team.starters === 5 ? "开赛 action 将使用当前服务器事实复核冻结名单、NJU 首发、外校实力、纪律与身份。" : "先完成合法首发后才能复核。" },
  ];
  ...
  const rosterBlockers = [teamA, teamB].flatMap((team) => !team.submitted ? [`${team.name} 尚未提交首发`] : team.starters !== 5 ? [`${team.name} 当前不是 5 名首发`] : !team.confirmed ? [`${team.name} 首发尚未确认`] : []);
