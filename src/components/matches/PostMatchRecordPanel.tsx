...
export type PostMatchRecordData = { commentators: { userId: string; name: string; hasLiveStream: boolean }[]; seasonAdmins: { userId: string; name: string; hasLiveStream: boolean }[]; submittedAt: Date | null; submittedByUserId: string | null; videoUrl: string | null; completionLabel: string };
export function PostMatchRecordPanel({ matchId, data }: { matchId: string; data: PostMatchRecordData }) {
  ...
  return <section className="space-y-3 border-t border-[var(--color-border)] pt-3">
    ...
    <div className="flex flex-wrap gap-2"><Input className="max-w-sm" type="url" placeholder="比赛录像链接（可在提交后补充）" value={videoUrl} onChange={(event) => setVideoUrl(event.target.value)} /><Button type="button" size="sm" variant="outline" disabled={isPending} onClick={() => run(() => updateMatchVideoUrl({ matchId, videoUrl }), "录像链接已保存。")}>保存录像</Button></div>
    {frozen ? <Button type="button" size="sm" variant="outline" disabled={isPending} onClick={() => run(() => revokePostMatchSubmission({ matchId }), "已撤销提交，可以重新编辑解说名单。")}>撤销提交 / 重新编辑</Button> : <Button type="button" size="sm" disabled={isPending || data.commentators.length === 0} onClick={() => run(() => submitPostMatchReport({ matchId }), "赛后资料已提交。")}>提交赛后资料</Button>}
  </section>;
}
