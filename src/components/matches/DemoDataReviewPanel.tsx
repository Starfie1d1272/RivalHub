...
import { confirmGameplaySteamIdentity, rejectStoredDemoImport } from "@/actions/demo-integration";
...
export function DemoDataReviewPanel({ reviews }: DemoDataReviewPanelProps) {
  ...
  return <DemoDataReviewPanelContent reviews={reviews} />;
}

function DemoDataReviewPanelContent({ reviews }: DemoDataReviewPanelProps) {
  ...
  function confirmIdentity(review: AdminDemoReviewMap, observedSteam64: string, eventRosterMemberId: string) {
    startTransition(async () => {
      const result = await confirmGameplaySteamIdentity({
        importId: review.importId,
        observedSteam64,
        eventRosterMemberId,
      });
      if (result.success) {
        toast.success(result.data.status === "confirmed" ? "比赛 Steam 身份已确认，Demo 数据已重新检查。" : "比赛 Steam 身份已保存，仍有其他数据需要处理。");
        router.refresh();
      } else {
  ...
  return (
    <Panel label="Demo 数据需要处理" contentClassName="space-y-5 p-4">
      <p className="text-sm leading-6 text-[var(--color-fg-mid)]">
        下面的 Demo 中发现了与报名资料不同的 Steam64 ID。请只在确认是同一位选手时保存映射；这不会修改选手登录或报名资料中填写的 Steam64 ID。
      </p>
      {reviews.map((review) => (
        <section key={review.importId} className="space-y-4 rounded border border-[var(--color-border)] p-4">
          <header className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-[var(--color-fg-mid)]">状态：需要处理</p>
            </div>
            <Button
              type="button"
              variant="outline"
                ...
                <div className="grid gap-3 text-sm md:grid-cols-2">
                  ...
                  <div>
                    ...
                    {participant.candidates.length === 0 ? (
                      <p className="mt-1 text-sm text-[var(--color-warn)]">本场首发名单暂无可选选手。</p>
                    ) : (
                        ...
                        {participant.candidates.map((candidate) => (
                          <div key={candidate.eventRosterMemberId} className="flex flex-wrap items-center justify-between gap-2 rounded bg-[var(--color-bg-soft)] p-2">
                            <div>
                              ...
                              <p className="mt-1 font-mono text-xs text-[var(--color-fg-mid)]">
                                报名资料中的 Steam64 ID：{candidate.steam64 ?? "未填写"}
                              </p>
                            </div>
                            <Button
                              type="button"
