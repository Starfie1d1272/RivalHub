    ...
    <div className="space-y-6">
      ...
      {recruitment && <Panel label="正在招募" pad={20}>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div className="space-y-3"><div className="flex flex-wrap items-center gap-2"><span className="text-sm text-[var(--color-fg-mid)]">需要位置</span>{recruitment.positions.length ? recruitment.positions.map((position) => <PosChip key={position} pos={position} />) : <span className="text-sm">位置不限</span>}</div>{recruitment.targetSeasonName && <p className="text-sm text-[var(--color-fg-mid)]">目标赛事 · {recruitment.targetSeasonName}</p>}{recruitment.note && <p className="max-w-2xl text-sm leading-6 text-[var(--color-fg-mid)]">{recruitment.note}</p>}<p className="text-xs text-[var(--color-fg-dim)]">更新于 {formatCSTShortDate(recruitment.updatedAt)}</p></div><RecruitmentInterestButton recruitmentIntentId={recruitment.id} interested={viewerInterested} loggedIn={loggedIn} />
        </div>
      </Panel>}
