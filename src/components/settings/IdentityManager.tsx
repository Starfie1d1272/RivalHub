...
export function IdentityManager({ identities }: { identities: Identity[] }) {
  ...
  function revoke(identity: Identity) {
    startTransition(async () => {
      ...
      toast.success("secondary email credential 已撤销；赛事历史与个人事实未改变。");
    });
  }

  return <div className="space-y-5">
    <Panel label="Verified email identities" contentClassName="p-0">
      <div className="divide-y divide-[var(--color-border)]">
        {identities.map((identity) => <div key={identity.id} className="flex flex-wrap items-center justify-between gap-3 p-4">
          <div>
            ...
            <p className="mt-1 text-xs text-[var(--color-fg-mid)]">{identity.primary ? "Primary login email" : "Secondary verified email"}</p>
          </div>
          {identity.primary
            ? <span className="font-mono text-[11px] text-[var(--color-fg-mid)]">PRIMARY</span>
            : <Button type="button" size="sm" variant="outline" disabled={pending} onClick={() => revoke(identity)}>撤销绑定</Button>}
        </div>)}
      </div>
    </Panel>
    <Panel label="添加 secondary email" contentClassName="p-5">
      <div className="space-y-4">
        <StatusBanner tone="info" title="先证明邮箱控制权" sub="验证不会更换 primary 登录邮箱。若该邮箱已属于另一个账号，将先显示 deterministic merge preflight，确认前不会写入赛事事实。" />
