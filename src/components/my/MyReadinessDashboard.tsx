  ...
  return (
    <div className="space-y-3 border border-[var(--color-border)] p-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        {responsibility && <span className="text-xs text-[var(--color-fg-dim)]">{responsibility === "self" ? "需要你处理" : `等待 ${presentMyReadinessResponsibility(responsibility)} 处理`}</span>}
...
export function MyReadinessDashboard({ model }: { model: MyWorkspaceModel }) {
  ...
  return <div className="space-y-8">
    <PageHeader eyebrow="MY RIVALHUB" title="我的参赛" description={`你好，${model.displayName}。这里先列出需要你处理的事项，再汇总当前队伍、赛事进度与长期资料；资料齐全不等于某届赛事一定可报名或出场。`} />
    ...
    <Section><SectionHeader title="当前参与" description="长期队伍关系与本届赛事身份分别展示。" /><TeamSummary model={model} />{model.currentCompetitions.length > 0 ? <div className="grid gap-4 xl:grid-cols-2">{model.currentCompetitions.map((context) => <MyCompetitionCard key={context.entryId} context={context} />)}</div> : <Panel contentClassName="p-5"><p className="text-sm text-[var(--color-fg-mid)]">当前没有负责或参与中的赛事。</p></Panel>}</Section>

    <Section><SectionHeader title="长期参赛资料" description="这些是持续维护的个人、教育和竞技事实；赛事会按当届规则单独核验。" /><div className="grid gap-4 md:grid-cols-2"><ReadinessCard item={readiness.profile} /><ReadinessCard item={readiness.education} />{readiness.competitiveProfiles.map((profile) => <CompetitiveCard key={profile.key} profile={profile} />)}</div></Section>
