...
describe("MetricValue sample labels", () => {
  it("shows undefined rates as an em dash while keeping the 0/0 sample visible", () => {
    ...
    expect(screen.getByText("—")).toBeInTheDocument(); expect(screen.getByText("0/0 attempts")).toBeInTheDocument();
  });
  it("renders a zero rate when the denominator is nonzero", () => {
    ...
    expect(screen.getByText("0.0%")).toBeInTheDocument(); expect(screen.getByText("0/8 rounds")).toBeInTheDocument();
  });
