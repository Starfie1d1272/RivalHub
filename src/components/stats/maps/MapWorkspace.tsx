...
export function MapWorkspace({ detail }: { detail: TournamentMapDetail }) {
  ...
  const teamColumns: StatsDataColumn<TeamRow>[] = [
    ...
    { key: "pistol", label: "Pistol%", metric: "pistol", numeric: true, className: "hidden lg:table-cell", sortable: true, sortValue: (row) => row.pistol.rate, rankingSample: (row) => statsRateDenominator(row.pistol), render: (row) => <MetricValue metric="pistol" value={row.pistol} sampleDisplay="compact" /> },
    { key: "opening", label: "Opening%", metric: "openingWin", numeric: true, className: "hidden lg:table-cell", sortable: true, sortValue: (row) => performanceByTeam.get(row.team.entityKey)?.slices.overall.opening.successRate.rate, rankingSample: (row) => { const value = performanceByTeam.get(row.team.entityKey)?.slices.overall.opening.successRate; return value ? statsRateDenominator(value) : null; }, render: (row) => { const value = performanceByTeam.get(row.team.entityKey)?.slices.overall.opening.successRate; return value ? <MetricValue metric="openingWin" value={value} sampleDisplay="compact" /> : "—"; } },
  ];
  const playerColumns: StatsDataColumn<typeof playersWithTeam[number]>[] = [
    ...
    { key: "opening", label: "Open Win%", metric: "openingWin", numeric: true, sortable: true, sortValue: (row) => row.slices.overall.opening.successRate.rate, rankingSample: (row) => statsRateDenominator(row.slices.overall.opening.successRate), render: (row) => <MetricValue metric="openingWin" value={row.slices.overall.opening.successRate} sampleDisplay="compact" /> },
  ...
  const weaponColumns: StatsDataColumn<WeaponRow>[] = [
    ...
    { key: "share", label: "Kill share", metric: "killShare", numeric: true, sortable: true, sortValue: (row) => row.killShare.rate, rankingSample: (row) => statsRateDenominator(row.killShare), render: (row) => <MetricValue metric="killShare" value={row.killShare} sampleDisplay="compact" /> },
    { key: "perRound", label: "Kills/R", metric: "killsPerRound", numeric: true, sortable: true, sortValue: (row) => row.killsPerRound.rate, rankingSample: (row) => statsRateDenominator(row.killsPerRound), render: (row) => <MetricValue metric="killsPerRound" value={row.killsPerRound} sampleDisplay="hidden" /> },
    { key: "hs", label: "HS%", metric: "headshot", numeric: true, sortable: true, sortValue: (row) => row.headshotRate.rate, rankingSample: (row) => statsRateDenominator(row.headshotRate), render: (row) => <MetricValue metric="headshot" value={row.headshotRate} sampleDisplay="compact" /> },
