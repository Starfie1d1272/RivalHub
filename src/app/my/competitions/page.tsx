...
export default async function MyCompetitionsPage() {
  ...
  return <div className="container mx-auto space-y-6 px-4 py-12 sm:py-16"><div className="flex flex-wrap items-end justify-between gap-3"><Marker sub="查看你负责或确认参与的赛事">我的赛事</Marker><Link className="text-sm text-[var(--color-accent)]" href="/my/teams">管理队伍 →</Link></div>{rows.length === 0 ? <Panel pad={24}><p className="text-sm text-[var(--color-fg-mid)]">你目前没有负责或确认参与的赛事。</p></Panel> : <div className="grid gap-4 sm:grid-cols-2">{rows.map((entry) => <Link key={entry.id} href={`/${entry.seasonSlug}/register`}><Panel className="h-full transition-colors hover:border-[var(--color-border-hi)]" pad={20}><div className="flex items-start justify-between gap-3"><div><p className="font-mono text-[10px] text-[var(--color-accent)]">赛事报名</p><h2 className="mt-1 text-lg font-semibold">{entry.name}</h2><p className="mt-1 text-sm text-[var(--color-fg-mid)]">{entry.seasonName}</p></div><span className="font-mono text-[10px]">{registrationLabels[entry.status]}</span></div><p className="mt-4 text-xs text-[var(--color-fg-mid)]">{entry.representativeUserId === session.userId ? "赛事负责人" : `已确认参赛 · ${entry.participantStatus === "confirmed" ? "已确认" : "待确认"}`} · 赛事 {entry.seasonStatus}</p></Panel></Link>)}</div>}</div>;
}
