...
export default async function MyTeamsPage() {
  ...
  const periods = await db.select({
    ...
    team: { id: teams.id, slug: teams.slug, name: teams.name, logoUrl: teams.logoUrl, description: teams.description, recruiting: teams.recruiting, captainUserId: teams.captainUserId, status: teams.status },
  ...
  return <div className="container mx-auto space-y-6 px-4 py-12 sm:py-16"><div className="flex flex-wrap items-end justify-between gap-3"><Marker sub="管理队伍资料、成员与邀请">我的队伍</Marker><div className="flex flex-wrap items-center gap-3">{current && <Button size="sm" variant="outline" asChild><Link href={`/teams/${current.team.slug}`}>查看队伍主页 →</Link></Button>}<Link className="text-sm text-[var(--color-accent)]" href="/my/competitions">查看我的赛事 →</Link></div></div><LongLivedTeamWorkspace currentUserId={session.userId} team={current ? toLongLivedTeamDto(current.team) : null} memberships={members} incomingInvitations={incoming.map((item) => ({ ...item, expiresAt: item.expiresAt.toISOString() }))} outgoingInvitations={outgoing.map((item) => ({ ...item, expiresAt: item.expiresAt.toISOString() }))} />{periods.length > 0 && <Panel label="成员历史" pad={20}><div className="space-y-2">{periods.map((row) => <Link key={row.membership.id} href={`/teams/${row.team.slug}`} className="flex flex-wrap justify-between gap-2 border-b border-[var(--color-border)] py-2 text-sm"><span>{row.team.name} · {row.team.captainUserId === row.membership.userId ? "队长" : "成员"}</span><span className="text-[var(--color-fg-mid)]">{formatCSTShortDate(row.membership.startedAt)} — {row.membership.endedAt ? formatCSTShortDate(row.membership.endedAt) : "至今"} · {presentTeamMembershipStatus(row.membership.status).label}</span></Link>)}</div></Panel>}</div>;
}
