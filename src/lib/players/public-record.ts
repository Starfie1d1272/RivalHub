...
import { matchRosters, matchRosterPlayers, eventRosterMembers, matches, seasons } from "@/db/schema";
...
export async function getPublicPlayerRecord(userId: string) {
  const appearances = await db.select({ matchId: matches.id, entryId: matchRosters.entryId, entryAId: matches.entryAId, entryBId: matches.entryBId, scoreA: matches.scoreA, scoreB: matches.scoreB })
    ...
    .where(and(eq(eventRosterMembers.userId, userId), eq(matchRosterPlayers.isStarter, true), eq(matchRosters.status, "confirmed"), eq(matches.status, "finished"), ne(seasons.status, "draft")));
  let totalWins = 0;
  let totalLosses = 0;
