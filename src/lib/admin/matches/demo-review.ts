...
export async function loadAdminDemoReview(
  ...
  try {
    ...
    for (const participant of evidence.participants) {
      ...
      const state = resolution ? (retirable ? "conflict-retirable" : "conflict-nonretirable") : confirmable ? "confirmable" : "blocked";
      participants.push({
        ...
        note: state === "confirmable" ? null : state === "conflict-retirable"
        ...
        candidates: state === "blocked" ? [] : starters.map((member) => ({ eventRosterMemberId: member.eventRosterMemberId,
    ...
    const unresolved = participants.length - conflicts;
    const message = participants.length > 0
      ? `需要处理：${[unresolved > 0 ? `${unresolved} 名选手 Steam 身份未确认` : "", conflicts > 0 ? `${conflicts} 名选手 Steam64 已关联其他选手` : ""].filter(Boolean).join("；")}`
