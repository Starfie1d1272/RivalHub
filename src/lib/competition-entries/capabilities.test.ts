...
describe("registration action capabilities", () => {
  ...
  it("allows an approved participant to withdraw while the roster-change window is open", () => {
    ...
    expect(getCompetitionEntryCapabilities(input, new Date("2026-09-20"))).toMatchObject({ canWithdrawParticipation: false, readOnlyReason: "名单调整已截止" });
    expect(getCompetitionEntryCapabilities({ ...input, rosterFrozen: true }, now)).toMatchObject({ canWithdrawParticipation: false, readOnlyReason: "最终名单已锁定" });
  });
