...
): { state: "pending" | "settled" | "refunded"; winningOptionIds: string[]; factRevision: string } {
  if (fact.state !== "confirmed")
    return { state: fact.state === "void" ? "refunded" : "pending", winningOptionIds: [], factRevision: fact.revision };
  const winners = [...new Set(fact.winningKeys)].sort();
  if (!winners.length || winners.some((key) => !options.some((option) => option.key === key)))
    throw new Error("Confirmed outcome is not represented by this market");
  return {
    state: "settled",
    winningOptionIds: options.filter((option) => winners.includes(option.key)).map((option) => option.id).sort(),
