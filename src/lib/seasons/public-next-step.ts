...
async function getPlayingSeasonNextStep(season: PublicSeason, userId: string) {
  ...
  return presentPersonalMatchTask({ matchId: match.id, seasonSlug: season.slug, opponentName: opponent.name, scheduledAt: match.scheduledAt, status: match.status === "in_progress" ? "in_progress" : "scheduled" });
}
