...
async function loadStatsEvidence(tx: TxDb, scope: TournamentStatsScope, options: { selectedMapOnly?: boolean } = {}) {
  const selectedMapRows = options.selectedMapOnly
    ? await tx.select({ match: matches, map: matchMaps }).from(matchMaps).innerJoin(matches, eq(matches.id, matchMaps.matchId)).where(and(
      ...
      eq(matchMaps.mapName, scope.map!),
      scope.stage ? eq(matches.stage, scope.stage) : undefined,
      scope.team ? or(eq(matches.entryAId, scope.team), eq(matches.entryBId, scope.team)) : undefined,
    ))
    : undefined;
...
export async function getTournamentStats(scope: TournamentStatsScope, database: DB = db) {
  return database.transaction(async (tx) => {
    ...
    const selection = buildVetoSelection(loaded.entries, veto, scope.map);
    return {
      leaderboard: await getStatsLeaderboard(scope.seasonId, scope.stage ?? "", scope.map ?? "", scope.team ?? "", loaded.selected.map((row) => row.importId), loaded.roster, tx),
      ...
      selection,
      coverage: { confirmedMaps: loaded.selected.length, completedMaps: loaded.scopedMaps.filter((map) => map.completedAt !== null && map.scoreA !== null && map.scoreB !== null).length },
      options: { teams: loaded.entries, maps: [...new Set([...loaded.maps.map((map) => map.mapName), ...veto.map((row) => row.mapName)])].sort() },
    };
  }, { isolationLevel: "repeatable read", accessMode: "read only" });
}

export async function getTournamentMapDetail(scope: Omit<TournamentStatsScope, "map"> & { map: string }, database: DB = db) {
  return database.transaction(async (tx) => {
    const loaded = await loadStatsEvidence(tx, scope, { selectedMapOnly: true });
    const performance = buildTournamentPerformanceAnalytics(loaded.selected.map((row) => row.facts.performance), { labels: loaded.labels });
    return { coverage: { confirmedMaps: loaded.selected.length }, performance: { weapons: performance.weapons } };
  }, { isolationLevel: "repeatable read", accessMode: "read only" });
}
