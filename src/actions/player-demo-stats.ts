...
import { demoPlayers, demoPlayerStats, demoBlinds, matchMaps } from "@/db/schema/demo";
...
export async function getPlayerDemoStats(
  ...
  if (!season) {
    return { ok: false, error: "赛季不存在" };
  }

  // 2. 找该 userId 在当前赛季 map 中参与的所有 demo 行
  const mapRows = await db
    .select({ mapId: matchMaps.id })
    .from(matchMaps)
    .innerJoin(seasons, eq(matchMaps.seasonId, seasons.id))
    .where(eq(seasons.slug, seasonSlug));
