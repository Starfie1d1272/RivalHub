-- Generated snapshot also normalizes pre-existing metadata drift: admin invite and announcement nullable fields (0000/0050), retired Steam actor FK RESTRICT (0052). No SQL change to those existing tables.
...
--> statement-breakpoint
CREATE TABLE "prediction_market_options" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL,
	...
	CONSTRAINT "prediction_market_options_market_id_key_unique" UNIQUE("market_id","key"),
	CONSTRAINT "prediction_market_options_id_market_id_unique" UNIQUE("id","market_id"),
	CONSTRAINT "prediction_market_options_market_id_position_unique" UNIQUE("market_id","position")
);
--> statement-breakpoint
CREATE TABLE "prediction_markets" (
	...
	CONSTRAINT "prediction_markets_match_id_resolver_unique" UNIQUE("match_id","resolver")
);
--> statement-breakpoint
...
--> statement-breakpoint
--> statement-breakpoint
-- rivalhub:migration-risk: locking-reviewed New empty prediction tables; short FK metadata locks, no rewrite of existing rows.
...
-- rivalhub:migration-risk: locking-reviewed New empty prediction tables; short FK metadata locks, no rewrite of existing rows.
ALTER TABLE "prediction_market_options" ADD CONSTRAINT "prediction_market_options_market_id_prediction_markets_id_fk" FOREIGN KEY ("market_id") REFERENCES "public"."prediction_markets"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
-- rivalhub:migration-risk: locking-reviewed New empty prediction tables; short FK metadata locks, no rewrite of existing rows.
...
-- rivalhub:migration-risk: locking-reviewed New empty prediction tables; short FK metadata locks, no rewrite of existing rows.
ALTER TABLE "prediction_stage_milestones" ADD CONSTRAINT "prediction_stage_milestones_season_id_prediction_programs_season_id_fk" FOREIGN KEY ("season_id") REFERENCES "public"."prediction_programs"("season_id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
-- rivalhub:migration-risk: locking-reviewed New empty prediction tables; short FK metadata locks, no rewrite of existing rows.
...
-- rivalhub:migration-risk: locking-reviewed New empty prediction tables; short FK metadata locks, no rewrite of existing rows.
ALTER TABLE "prediction_stakes" ADD CONSTRAINT "prediction_stakes_option_id_market_id_prediction_market_options_id_market_id_fk" FOREIGN KEY ("option_id","market_id") REFERENCES "public"."prediction_market_options"("id","market_id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
--> statement-breakpoint
...
--> statement-breakpoint
ALTER TABLE "prediction_market_options" ENABLE ROW LEVEL SECURITY;
REVOKE ALL PRIVILEGES ON TABLE "prediction_market_options" FROM anon, authenticated;
--> statement-breakpoint
...
DO $$ DECLARE tab text; BEGIN
  FOREACH tab IN ARRAY ARRAY['picks','judgements','stakes','settlements','ledger','scenarios','accounts','market_options','stage_milestones'] LOOP
...
--> statement-breakpoint
CREATE TRIGGER prediction_official_outbox AFTER INSERT OR DELETE OR UPDATE OF status,score_a,score_b,completed_at,entry_a_id,entry_b_id,scheduled_at,major_stage_run_id,stage ON public.matches FOR EACH ROW EXECUTE FUNCTION public.rivalhub_enqueue_predictions();
--> statement-breakpoint
...
--> statement-breakpoint
CREATE TRIGGER prediction_official_outbox AFTER INSERT OR UPDATE OR DELETE ON public.major_stage_entrants FOR EACH ROW EXECUTE FUNCTION public.rivalhub_enqueue_predictions();
