...
当前阶段：**v1.3.2 生产运行中，站点部署在 `match.starfie1d.top`。v2 赛制引擎（StageExecutor + 5 个 executor + entrySeeds 种子轮空 + finalFormat 决赛 BO5）代码已就绪，待 2026 NJU Major 赛季开始时启用。**
...
发布版本时必须递增版本号，由发布者判断层级，并在版本提交上打 tag `vX.Y.Z`。

**禁止手动修改 `package.json` 中的 version 字段。** 必须使用 `npm version <patch|minor|major|x.y.z>`，让 npm 同步生成版本提交与 tag。
